...
const contentTypeProps = {'em' : {'legend': 'EM Layers'}, 
...
function LayerCheckboxList({ volumes, checkState, handleChange }: LayerCheckBoxListProps) {
    ...
    return <React.Fragment key={contentType}><FormLabel component="legend">{contentTypeProps[contentType].legend}</FormLabel>
                <Divider />
                <FormGroup className={classes.formGroup}>{checkBoxList}</FormGroup></React.Fragment>
}

export default function LayerCheckboxListCollection({ dataset, checkState, handleChange }: LayerCheckBoxListCollectionProps) {
    const classes = useStyles();
    ...
    dataset.volumes.forEach((v: Volume) => {
        if (volumeGroups.get(v.contentType) === undefined){volumeGroups.set(v.contentType, [])}
    ...
    <FormControl component="fieldset" className={classes.formControl}>{checkboxLists}</FormControl>
    </Grid>
);
};
