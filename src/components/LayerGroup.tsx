...
export default function VolumeCheckboxCollection({
  ...
  if (!(layerTypeToggleLabel === undefined) && !(handleLayerChange === undefined)) {
    layerTypeSwitch = LayerTypeSwitch({label: layerTypeToggleLabel, checked: checkState.get(volumes[0].name)?.layerType === 'segmentation', contentType: contentType, handleLayerChange: handleLayerChange});
  }
  else {
